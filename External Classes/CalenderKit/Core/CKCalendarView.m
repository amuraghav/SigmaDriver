...
-(void)removeFromSuperview
{
    ...
    [[self headerView] setDelegate:nil];
...
- (void)setDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    if ([[self delegate] respondsToSelector:@selector(calendarView:tableViewIsReoloadedTable:)]) {
        [[self delegate] calendarView:self tableViewIsReoloadedTable:[self table]];
    }
...
//            UIImageView  *accessoryImage = [[UIImageView alloc]initWithFrame:CGRectMake(cell.frame.size.width-30,36,20, 20)];
//            accessoryImage.image = [UIImage imageNamed:@"home_cell_arrow.png"];
//            
//            [cell.contentView addSubview:accessoryImage];
        ...
        [cell.docApptImage setImageWithURL:[NSURL URLWithString:strImageUrl]
                      placeholderImage:[UIImage imageNamed:@"home_image_thumbnail.png"]
        ...
        [cell.separator setTextColor:[UIColor blackColor]];
        [cell.appDateTime setText:[event appDate]];
        [cell.appDateTime setText:[event time]];
        [cell.appDateTime setTextColor:[UIColor blackColor]];
       ...
       cell.imgDropLocation.textColor=[UIColor redColor];
        cell.imgPickLocation.textColor=[UIColor redColor];
        cell.totalAmount.text = [NSString stringWithFormat:@"$ %@",[event amount]];
        cell.totalAmount.textColor=[UIColor blackColor];
        cell.distance.text = [NSString stringWithFormat:@"%@ %@",[event distance],@"Miles"];
        cell.distance.textColor=[UIColor blackColor];
        [cell.status setText:[event status].uppercaseString];
        cell.status.textColor=UIColorFromRGB(0x63b827);
        switch ([[event.info objectForKey:@"payStatus"] intValue]) {
            case kPaymentStatusDone:
            {
                cell.paymentStatus.text = @"PAID";
                break;
            }
            case kPaymentStatusNotDone:
            {
                cell.paymentStatus.text = @"NOT PAID";
                break;
            }
            case kPaymentStatusDisputRaised:
            {
                cell.paymentStatus.text = @"DISPUTED";
                break;
            }
            case kPaymentStatusClosed:
            {
                cell.paymentStatus.text = @"CLOSED";
                break;
            }
            default:
                break;
