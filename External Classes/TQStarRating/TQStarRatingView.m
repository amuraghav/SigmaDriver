...
- (id)initWithFrame:(CGRect)frame numberOfStar:(int)number starSize:(CGSize)size selectedImage:(NSString*)sImage unselectedImage:(NSString*)unImage
{
    self = [super initWithFrame:frame];
    if (self) {
        _numberOfStar = number;
        self.starBackgroundView = [self buidlStarViewWithImageName:sImage];
        self.starForegroundView = [self buidlStarViewWithImageName:unImage];
        [self addSubview:self.starBackgroundView];
        [self addSubview:self.starForegroundView];
    }
    return self;
